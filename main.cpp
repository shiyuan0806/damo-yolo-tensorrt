...
//    int detection_labels_index = engine->getBindingIndex("detection_classes");
    ...
    int num_detections_index = engine->getBindingIndex("num_dets");
    int detection_boxes_index = engine->getBindingIndex("det_boxes");
    int detection_scores_index = engine->getBindingIndex("det_scores");
    int detection_labels_index = engine->getBindingIndex("det_classes");
    ...
    tmp.convertTo(tmp, CV_32FC3, 1.0);  // damo-yolo 这边要改一下，damo yolo 好像没做归一化
    {
        /* HWC ==> CHW */
        int offset = 0;
    ...
    std::cout << "inference time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() /10.0 << "ms" << std::endl;
